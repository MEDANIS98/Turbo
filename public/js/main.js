		...
		$('#select-year').on('change', (event) => {
			...
			$.post('/api/getYearBrands', {
				...
				$.each(data, (brand) => {
					...
					$('#select-brand').append($(option).attr('value', data[brand]).html(data[brand]));
				});
				$('#select-brand').prop('disabled', false).val('none');
			})
		});
		$('#select-brand').on('change', (event) => {
			var selectedBrand = $('#select-brand').find(":selected").text();
	...
	*/
	$(function () {
		$('#select-category').on('change', function () {
			...
			$.post('/api/getCategoryTypes', {
				category: selectedCategory
			}, (data) => {
				var defaultOption = $('#select-type').children(":first")[0].outerHTML;
				$('#select-type').empty();
				$('#select-type').append(defaultOption);
				$.each(data, (type) => {
					var option = document.createElement('option');
					$('#select-type').append($(option).attr('value', data[type].id).html(data[type].name));
				});
				$('#select-type').prop('disabled', false).val('none');
			})
		});
	});
})(jQuery);
